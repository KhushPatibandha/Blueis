...
func HandleAppend(connection net.Conn, server *typestructs.Server, parts []string, setGetMap map[string]string, expiryMap map[string]time.Time, connAndCommands map[net.Conn][]string, dataStr string, dir string, dbfilename string, flag bool) string {
	if flag {
	...
	if ok {
		...
		if ok && time.Now().After(expiry) {
			...
			dataToSend := ":" + strconv.Itoa(len(parts[6])) + "\r\n";
		...
		setGetMap[key] = keyToGet + parts[6];
		dataToSend := ":" + strconv.Itoa(len(setGetMap[key])) + "\r\n";
		if flag {
			_, err := connection.Write([]byte(dataToSend));
			if err != nil {
				fmt.Println("Error writing:", err.Error());
